...
| Business directory (mobile parity added 2026-07-17; mobile rebuilt 2026-09-25, app v1.1.0) | §8-Businesses | `routers/businesses.py` (list takes `category_slug`, `locality_slug`, `q`, `page`/`page_size`; `/businesses/counts`) | Web: `[city]/businesses/page.tsx`, `BusinessDetailClient.tsx`. Mobile: every category tile on `HomeScreen.tsx` except Classifieds/Events opens `BusinessesScreen.tsx` filtered to that category (chips with live counts, paging, covers via `mobile/src/lib/categoryCover.ts`, "Open now", "Classified ads for X →" link); home has a "Popular in {city}" business row | `businesses`, `reviews` | GET /businesses, GET /businesses/counts, POST /businesses |
| Business profile | §8-BusinessProfile | `routers/businesses.py` | `[city]/businesses/[id]/page.tsx`, `BusinessDetailClient.tsx` (web); `mobile/src/screens/BusinessDetailScreen.tsx` (mobile; 2026-09-25: labelled category cover, hours + Open now via `mobile/src/lib/openingHours.ts`, Call/Directions/Share, claim sheet, owner review-invite card, OSM attribution, paid badge gated by `PAID_BADGES_ENABLED`; fixed `website_url`/`category_slug` field names) | `businesses`, `reviews` | GET /businesses/{id} (includes `city_slug`, `locality`, `opening_hours`) |
| Claim business (2026-09-24: SMS OTP to the business's listed mobile = instant; or document + shopfront photo + optional visiting card + callback phone → admin review; old instant first-come claim removed) | §8-BusinessProfile | `routers/business_claims.py`, `services/cloudinary_svc.py` (`upload_private_image`/`private_image_url` — Cloudinary type=private, 10-min signed download URLs) | `components/claim-business/ClaimBusinessModal.tsx`, `BusinessDetailClient.tsx`, `app/admin/business-claims/page.tsx`; mobile `components/ClaimBusinessSheet.tsx` (2026-09-25 — documents upload via axios, Expo 56 fetch rejects RN file parts) | `business_claims`, `businesses.owner_id` | GET /businesses/{id}/claim-options, POST /businesses/{id}/claim/otp/send, POST /businesses/{id}/claim/otp/verify, POST /businesses/{id}/claim/documents, GET /admin/business-claims, POST /admin/business-claims/{id}/approve, POST /admin/business-claims/{id}/reject |
| Owner outreach (2026-09-25: admin calls/WhatsApps unclaimed businesses one at a time — pre-filled wa.me message, sent manually, no bulk — and logs the outcome; queue buckets To contact / Follow up / Closed / Claimed, mobiles first) | §8-Admin | `routers/business_outreach.py` | `app/admin/outreach/page.tsx`, `lib/outreach.ts` | `business_outreach` | GET /admin/outreach, POST /admin/outreach/{business_id} |
| Neighbourhood pages (2026-09-25: each business tagged with its nearest OpenStreetMap suburb/neighbourhood; `/[city]/area/[area]` + `/[city]/[category]/[area]` pages, indexed at 3+ businesses, own sitemap) | §8-SEO | `routers/businesses.py` (localities, locality-pages), `routers/business_import.py` (import/localities), `agents/assign_localities.py`, workflow `assign-localities.yml` | `app/[city]/area/[area]/page.tsx`, `app/[city]/[category]/[area]/page.tsx`, `app/sitemap-areas.xml/route.ts`, `components/business-list/BusinessList.tsx`, `components/city-explore/CityExplore.tsx` | `businesses.locality`, `businesses.locality_slug` | GET /businesses/localities, GET /businesses/locality-pages, POST /admin/businesses/import/localities |
| Opening hours (2026-09-25: OSM `opening_hours` stored raw; business page shows hours + Open now/Closed in India time, JSON-LD openingHours; unreadable formats shown as written) | §8-BusinessProfile | `routers/business_import.py` (import/hours), `agents/osm_business_import.py --backfill-hours` | `lib/openingHours.ts`, `BusinessDetailClient.tsx`, `app/[city]/businesses/[id]/page.tsx` | `businesses.opening_hours` | POST /admin/businesses/import/hours |
...
| `lib/jsonLd.ts` | `serializeJsonLd()` — escapes `< > &` and U+2028/9; **every** `<script type="application/ld+json">` must use it (stored-XSS fix 2026-09-25) |
| `lib/imageLoader.ts` | next/image custom loader: Cloudinary serves each photo at the displayed width with `f_auto,q_auto` (SWA can't run Next's optimiser) |
| `lib/openingHours.ts` | Parses OSM `opening_hours` (common formats), `openStatus()` in India time, `describeWeek()`, `schemaOpeningHours()` for JSON-LD (copied to `mobile/src/lib/`) |
| `lib/features.ts` | `PAID_BADGES_ENABLED` switch (false — paid "Get Verified" offers hidden; mirrored in `mobile/src/lib/features.ts`) |
| `lib/outreach.ts` | Owner-outreach helpers: invite message, `wa.me` link, outcome labels |
...
GET    /api/v1/businesses/locality-pages?city_slug=  area pages with 3+ businesses (city-wide + per category) for sitemap-areas.xml
POST   /api/v1/admin/businesses/import/localities  {city_slug, items:[{id, locality|null}]} ≤2000 — set neighbourhood [ADMIN]
POST   /api/v1/admin/businesses/import/hours  {items:[{source_ref, opening_hours}]} ≤2000 — fills only empty hours [ADMIN]
...
GET    /api/v1/admin/outreach?city_slug=&category_slug=&status=&page=  Outreach queue (unclaimed businesses with a phone) + per-status counts [ADMIN]
POST   /api/v1/admin/outreach/{business_id}  {outcome, note?} — log one manual call/WhatsApp (whatsapp_sent/no_answer/callback/interested/not_interested/wrong_number) [ADMIN]
...
| `businesses` | id, city_id, owner_id, name, address, phone, whatsapp_url, verified, avg_rating, review_count, deleted_at, source, source_ref (unique), latitude, longitude, locality, locality_slug (migration `b6c7d8e9f0a1`, set by `agents/assign_localities.py`), opening_hours (OSM syntax, migration `c8d9e0f1a2b3`) | avg_rating recalculated on review. source='osm' rows come from `agents/osm_business_import.py` (migration `c5e6f7a8b9d0`) and must show `OsmAttribution` (ODbL) |
...
| `business_outreach` | id, business_id, admin_id, outcome, note, created_at | Migration `f2a3b4c5d6e7`. Append-only log of manual owner outreach; latest row per business = its status (closed = not_interested/wrong_number; claimed = owner_id set after contact) |
...
| `mobile/src/components/ClaimBusinessSheet.tsx` | (2026-09-25) Claim a business in the app: SMS code / documents (doc type + document + shop front + optional visiting card) / email; keyboard-height lift (KeyboardAvoidingView mis-measured inside the Modal) |
| `mobile/src/components/BusinessRow.tsx` | (2026-09-26) Horizontal business-card row (cover, name, category, View all → Businesses) shared by `HomeScreen` "Popular in city" and `SearchScreen` results |
| `mobile/src/components/ReviewInviteCard.tsx` | (2026-09-25) Owner-only "Get reviews from your customers" WhatsApp/share card |
| `mobile/src/lib/categoryCover.ts`, `coverPools.ts`, `openingHours.ts` | (2026-09-25) COPIES of the web `frontend/src/lib/` files — keep in sync; covers load from www.localsindia.com via `siteImage()` |
| `mobile/src/lib/features.ts` | `PAID_BADGES_ENABLED` (false), `SITE_URL`, `siteImage()` |
| `mobile/src/lib/businessCategories.ts` | Business category slug → label, shared by the list and detail screens |
...
**To run:** `cd mobile && npm install && npx expo start` (against a local backend: `EXPO_PUBLIC_API_URL=http://127.0.0.1:8000/api/v1 npx expo start --dev-client` + `adb reverse tcp:8000 tcp:8000` — testing only, release builds use production)
**Play releases:** versionCode 12 (2026-08-10, first production), 13 (2026-08-14), 14 (2026-08-31), 15 = v1.1.0 (2026-09-25, business directory/claim/hours) — see PROJECT_MAP.md §6.
