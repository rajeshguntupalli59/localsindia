...
### `business_images` / `event_images` — photo upload for Business Directory and Events (added 2026-07-22)

Both are 1:1-per-parent extensions with the exact same shape as `listing_images` (`id`, parent FK with `ondelete=CASCADE`, `url`, `cloudinary_id`, `display_order`, `created_at`) — businesses/events previously had no photo storage at all. `Business.images` and `Event.images` relationships use `lazy="selectin"`, so `BusinessOut`/`EventOut` responses include `images` automatically on every query (list, create, get) with no router changes needed — same effect as an explicit `.options(selectinload(...))` but declared once on the model instead of at every call site.

Migration `bc6a44aafa08`. Uploaded via `POST /upload/business-image/{business_id}` / `POST /upload/event-image/{event_id}` (see §6) — same 5MB/5-image limits as listings, validated by a new shared `_validate_and_upload()` helper in `routers/uploads.py` (factored out once this became the third near-identical copy of that logic).
...
| POST | `/business-image/{business_id}` | Upload business photo (2026-07-22), max 5 | Yes, owner/admin |
| DELETE | `/business-image/{image_id}` | Delete business photo | Yes, owner/admin |
| POST | `/event-image/{event_id}` | Upload event photo (2026-07-22), max 5 | Yes, owner/admin |
| DELETE | `/event-image/{image_id}` | Delete event photo | Yes, owner/admin |
...
The entry point. Restructured 2026-07-22 — previously a generic hero → flat category grid → trust-badges template; now organized around directly answering "why LocalsIndia" instead of a features-list shape:
1. **Hero**: dynamic headline (cycles category words), search bar + city selector + geolocation button. Redirects straight to the city page if a city is already saved (`li_city` in localStorage).
2. **"Why LocalsIndia"**: 4 concrete differentiators, not vague trust badges — Everything in one place, Talk directly no middlemen, Built for your neighbourhood, In your language (`buildWhyUs()`, city/language counts are real, derived live from `usePrefs()`/`LANGUAGES`, never hardcoded).
3. **"A Day in [City]"**: the same 8 categories, grouped by time-of-day (`DAY_SECTIONS` — Morning/Midday/Evening/Anytime) instead of a flat icon grid, each with a one-line editorial frame. Deliberately not a dynamic category count from `/api/v1/categories` — a curated subset chosen to fit the four time buckets.
4. **Fresh Listings** (`FreshListingsSection`, unchanged): real listings from the visitor's city with an honest fallback — shows labeled "Example listings" placeholders and a "no live postings yet, be the first to post one" footnote when a city has no real data yet, rather than fabricating activity.
5. **Closing CTA**: single decisive close (WhatsApp-native claim + real city/language stats + Post/Browse buttons) — replaces the old 4-card dark trust-badges grid, which restated the same claims from section 2.
...
Form to add a new business listing: name, category, description, address, phone, WhatsApp, and up to 5 photos (added 2026-07-22 — uploaded via `api.upload.businessImage` right after the business is created, same picker UI as `classifieds/post`). On success, switches into a "Business listed!" upsell screen (`createdBusinessId` state) offering `GetVerifiedModal` ("Get Verified Now") or "Skip for now — my business is already listed" (explicit about the business already being live, not just closing a dialog).
...
Form: title, description, venue, date+time picker, free/paid toggle, ticket URL, and up to 5 photos (added 2026-07-22 — uploaded via `api.upload.eventImage` right after the event is created).
...
- **Purchase flow**: `POST /api/v1/tickets/create-order` → `POST /api/v1/tickets/verify` (HMAC-checked exactly like `payments.py`'s `verify_featured_payment`). Since the 2026-09-25 security fix, `create-order` also stores a `PaymentOrder(kind='event_ticket', target_id=event)` and `verify` takes the event + amount from that record (not the request), requires the caller to own it and marks it consumed — one payment can't mint tickets for other events or be replayed. Only `active` (approved) events sell tickets; `EventUpdate` no longer accepts `status`. Web opens the standard Razorpay Checkout JS; mobile reuses the same `WebView` + injected-HTML-with-`postMessage` bridge pattern already established by `PromoteScreen.tsx` and `BusinessDetailScreen.tsx` — no new payment mechanism invented.
...
| `mobile/src/screens/PostScreen.tsx` | Dynamic 4–5 step wizard (restructured 2026-07-21): category grid (own step, nothing else) → category-specific questions *(skipped for Classifieds/Businesses/Events via `hasDetailsStep`)* → title/description/price/area (or event fields, `LISTING_COPY` per category) → photos (expo-image-picker, real upload zone for all categories incl. Businesses/Events since 2026-07-22 — previously a "coming soon" placeholder) → contact. `CATEGORY_DETAIL_FIELDS` + `renderDetailField()` render per-category text/number/select/multiselect/switch inputs; `buildCategoryDetailsPayload()` sends the result as `category_details` on create; `uploadPhoto()` takes an endpoint segment (`image`/`business-image`/`event-image`) so the same upload loop works for all three entity types. "Include my location" toggle (2026-07-16) auto-fills Area if empty + auto-picks City by exact name match against the seeded list (never forces an unmatched city), both always manually editable; (2026-07-17) picking "Businesses" as the category branches submit() to create a real Business Directory entry (`businessesApi.create`) instead of a classified — same wizard, Price hidden, Area relabels to Address; uses `navigation.navigate` (not `replace`, fixed 2026-07-22 — `replace` from inside the tab navigator was wiping the whole tab stack, breaking the back button on `BusinessDetailScreen`) |
| `mobile/src/screens/BusinessesScreen.tsx` | (new, 2026-07-17; rebuilt 2026-09-25) Business Directory for the user's city — category chips with live counts (`/businesses/counts`), paged list (20/page, loads on scroll), cover photo per card, "Open now", optional `categorySlug` route param (home category tiles open it filtered); "Add Your Business" opens Post Listing with category pre-selected |
| `mobile/src/screens/BusinessDetailScreen.tsx` | Business page — labelled cover when no photos, hours + Open now (India time), Call / Directions (Google Maps) / Share, claim (`components/ClaimBusinessSheet.tsx`), owner review-invite card + View Analytics, OSM attribution, reviews; paid badge UI behind `PAID_BADGES_ENABLED` (false) |
...
*Updated: 2026-07-26 | Mobile location system unified into one flow (§14 cross-reference; full detail in `ARCHITECTURE_INDEX.md`'s Feature Map, this file's mobile narrative sections were already flagged stale below and are not being backfilled here beyond this log entry) — `PostScreen.tsx`'s separate "Include my location" GPS toggle and "set exact location on map" link collapsed into one row, since `MapPinPickerScreen.tsx` now fetches GPS itself when opened without a known coordinate (previously showed a useless static India-wide view if opened cold) and live-refreshes an editable area/locality field on every pin move instead of reverse-geocoding once at confirm time. Pattern modeled directly on Swiggy/OLX/Uber's rural/village addressing UX per Raj's request: GPS only ever decides where the pin starts, confirming it (auto or dragged) is the only thing that sets a listing's location — no separate toggle needed. No new permission-prompt code: a fresh signup already lands on `HomeScreen` right after `navigation.replace('Main')`, which already fires the one-time `CityAutoDetectPrompt` (no saved city yet), and the OS permission that grants/denies is reused silently by every later location call since `requestForegroundPermissionsAsync()` is already idempotent. Also fixed same session: the Google Maps API key was being silently stripped from every mobile build by a config-plugin mismatch (`react-native-maps` reads a plugin option, not the static `android.config.googleMaps.apiKey` field this project was setting) — found by decompiling the built APK's manifest, commit `1ed7bc4`; and new-signup name collection, which had silently never fired for any user because `/auth/password/set` hardcoded `is_new_user=False`, commit `fe4b7d5`. Verified live on an Android emulator end-to-end; `tsc` clean, no native rebuild required. Previous update: 2026-07-22 | Homepage restructured (§8) — replaced the generic hero → flat category grid → trust-badges template with a structure organized around directly answering "why LocalsIndia": hero (kept) → explicit "Why LocalsIndia" differentiators section (4 concrete claims, not vague badges) → "A Day in Your City" (the same 8 categories regrouped by time-of-day instead of a flat icon grid) → Fresh Listings (unchanged, already had an honest real/mock-data fallback) → one decisive closing CTA (replaces the old 4-card dark trust-badges grid, which restated the same claims). Driven by direct user feedback that the previous structure — while individually well-crafted — was "the same template shape every marketplace site uses," and that the page needed to make the case for LocalsIndia explicit rather than implicit. No fabricated activity/stats introduced — all real-time claims (city count, language count) stay derived live from `usePrefs()`/`LANGUAGES`, matching the site's existing integrity bar. Verified via `tsc`, `next build`, `next lint`, and visual review with Playwright on the dev server. Previous update: 2026-07-22 | Photo upload for Businesses and Events (§5 `business_images`/`event_images`, §6 upload endpoints, §8 Add Business/Post Event/detail pages) — previously neither entity had any photo storage at all (the Photos step showed a "coming soon" placeholder); now both get the same 5-photo upload flow as classifieds listings, via new 1:1 extension tables mirroring `listing_images` and a shared `_validate_and_upload()` helper (third near-identical copy of that logic, finally factored out). 143/143 backend tests passing (5 new). Also today: two live bugs found and fixed — Events was missing from the category-aware `LISTING_COPY` map (mobile+web) and silently fell back to generic "What are you selling?" copy; and posting a Business used `navigation.replace` from inside the tab navigator, which wiped the entire tab stack and left `BusinessDetailScreen`'s back button with nothing to return to (now `navigate`, matching the convention used elsewhere in the app). Also today: category-aware copy for the generic Listing step (§8) — PG/Roommate, Education, etc. now get contextual title/description/price labels (e.g. "Monthly Rent") instead of one-size-fits-all "What are you selling?"; Jobs/Doctors hide the redundant generic price field since their category-specific questions already cover it. Also today: Get Verified badge purchase flow (§8, §12) — reachable inline right after creating a business, not just from a separate screen; "Cancel" relabeled to "Skip for now — my business is already listed" after user-reported confusion about whether declining the badge also cancelled the listing. Previous update: 2026-07-21 | Category-specific structured listing fields (§5 11 new `*_details` tables, §6 Listings endpoints, §8 Post Listing page) — the Post wizard on both web and mobile now picks a category first, then shows a dedicated step of category-specific questions (Vehicles, Jobs, PG/Roommate, Real Estate, Electronics, Furniture, Fashion, Education, Doctors, Services, Tiffin), separate from the generic Title/Description step, instead of asking for both at once; Classifieds/Businesses/Events skip that step since they have no specific fields (or, for Businesses/Events, their own dedicated tables already). Chose real typed columns per category over a flexible JSON blob, for future Search filterability. Web's old `CATEGORY_CHIPS`/`attributes` system — a much smaller per-category chip set the backend silently discarded, since `ListingCreate` never declared an `attributes` field — was replaced with the real `category_details` payload, matching the backend schema and the mobile field-set exactly. Backend: 138/138 tests passing (2 new). Verified live on the Android emulator (full create→review→My-Listings round trip) and via Playwright on the web dev server. Previous update: 2026-07-18 | Scoped to South India only (migration `a3b4c5d6e7f8`, §5 cities table) — reversible `cities.active` data change, no code touched, both web and mobile automatically reflect it via the shared `/api/v1/cities` endpoints. Google Auth removed from web login via its pre-existing feature flag (§11 updated — corrected a stale claim that mobile still had a working Google deep-link; it had already been silently removed from `LoginScreen.tsx` in an earlier, undated pass). Backend `/auth/google*` routes deliberately kept, not deleted, for one real Google-only user with no phone fallback. Also today: Mobile event creation (§12) — added to `PostScreen.tsx` right after the ticketing feature shipped, once Raj asked specifically whether mobile event creation could be added too; extends the wizard exactly like Businesses did, one new native dependency accepted (`@react-native-community/datetimepicker`) since a rebuild is already pending regardless. Also today: Event ticketing, web + mobile (§12) — second piece of Phase 3 monetization; new `tickets` table, `routers/tickets.py`, the first-ever web `/events/[id]` page, and mobile Events built from scratch (browse/detail/buy/my-tickets); QR generated server-side so mobile needed zero new native dependencies, deliberately avoiding a second pending native rebuild — see §12 for the full reasoning. 129/129 backend tests, frontend + mobile verification all clean. Also today: Business Analytics Dashboard (§12) — first piece of Phase 3 monetization, commit `63155f0`; verified live in production (migration ran clean via CI, new route confirmed present + auth-gated on the deployed backend). Also today: two-sided referral system (§5 users table, §6 Auth endpoints, §8 /invite page, §9 ReferralCapture component, §18 mobile App.tsx/LoginScreen/ProfileScreen/InviteScreen) — commits `1f7243c` (backend+web) and `ce2ee31` (mobile); real Play Console App Signing SHA-256 added to assetlinks.json, commit `3015c0a`. ⚠️ This pass only closes the referral-system gap specifically — several earlier feature passes (2026-07-12 through 2026-07-17: password auth rework, buyer requests, proximity search, business directory mobile parity, push notifications, error tracking) are still only reflected in `ARCHITECTURE_INDEX.md`, not fully backfilled into this file's narrative sections yet. Previous update: 2026-06-16 | LocalIndia v3 -- hybrid SSR migration, admin role management, business soft-delete, mobile Google Sign-In + admin panel + EAS Play Store pipeline + real logo assets + safe-area bottom nav fix*
